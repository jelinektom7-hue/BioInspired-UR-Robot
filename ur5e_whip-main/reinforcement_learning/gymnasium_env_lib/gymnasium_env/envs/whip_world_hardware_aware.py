...
class WhipWorldEnv(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        self.ENABLE_HARDWARE_AWARE_ACTION_FILTER = True

        # Conservative training limits, in rad/s and rad/s^2.
        # They are not exact UR factory limits; they are deliberately softer training limits
        # intended to avoid the shoulder-heavy, high-acceleration motions that caused
        # protective-stop warnings on the real robot.
        self.COMMAND_VELOCITY_LIMIT_RAD_S = np.array([1.40, 1.40, 1.70, 2.00, 2.40, 2.40], dtype=np.float64)
        self.COMMAND_ACCEL_LIMIT_RAD_S2 = np.array([8.0, 8.0, 10.0, 12.0, 14.0, 14.0], dtype=np.float64)

        # Soft reward thresholds. Going above these does not terminate the episode,
        # but it becomes increasingly expensive.
        self.SOFT_JOINT_VELOCITY_LIMIT_RAD_S = np.array([1.20, 1.20, 1.50, 1.80, 2.20, 2.20], dtype=np.float64)
        self.SOFT_COMMAND_ACCEL_LIMIT_RAD_S2 = np.array([6.0, 6.0, 8.0, 10.0, 12.0, 12.0], dtype=np.float64)
        self.SOFT_COMMAND_JERK_LIMIT_RAD_S3 = np.array([120.0, 120.0, 160.0, 200.0, 240.0, 240.0], dtype=np.float64)

        # Encourage the solution to use several joints instead of mostly shoulder_pan / shoulder_lift.
        self.MOBILITY_ACTIVE_JOINT_SPEED_RAD_S = 0.15
        self.SHOULDER_DOMINANCE_FRACTION = 0.72
    ...
    def calc_reward(self):
        ...
        action_change_penalty = -0.02 * np.sum(np.square(self._last_action - self._prev_action))
        ...
        joint_velocity_penalty = -0.20 * np.sum(np.square(measured_velocity_excess))
        ...
        command_velocity_penalty = -0.40 * np.sum(np.square(command_velocity_excess))
        ...
        command_acceleration_penalty = -0.20 * np.sum(np.square(command_acceleration_excess))
        ...
        command_jerk_penalty = -0.03 * np.sum(np.square(command_jerk_excess))
        ...
        action_filter_penalty = -4.0 * np.sum(np.square(self._action_clip_excess))
        ...
        if self._distance_to_target > 0.20 and not self._whip_target_contact:
            joint_participation_reward = 0.08 * min(self._active_joint_count, 4)
            ...
            if total_command_speed > 0.30 and self._shoulder_speed_fraction > self.SHOULDER_DOMINANCE_FRACTION:
                shoulder_dominance_penalty = -1.00 * np.square(
