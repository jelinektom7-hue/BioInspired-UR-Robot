...
MULTI_JOINT_REWARD_WEIGHT = 0.006
...
ACTION_ACTIVITY_REWARD_WEIGHT = 0.0
...
DISTANCE_REWARD_WEIGHT = 0.4
DISTANCE_REWARD_DECAY = 5.0
PROGRESS_REWARD_WEIGHT = 45.0
PROGRESS_CLIP_NEG = -0.03
PROGRESS_CLIP_POS = 0.05
CLOSEST_IMPROVEMENT_REWARD_WEIGHT = 25.0
...
# close misses.
NEAR_HIT_BONUS_25CM = 3.0
NEAR_HIT_BONUS_18CM = 10.0
NEAR_HIT_BONUS_12CM = 25.0
NEAR_HIT_BONUS_08CM = 60.0
...
HIT_DISTANCE_FALLBACK_M = 0.055  # kept for close-miss logging only; hit reward requires contact
...
DIRECTED_SWING_REWARD_WEIGHT = 20.0
...
EXPLORATION_MOTION_REWARD_WEIGHT = 0.0
EXPLORATION_WHIP_SPEED_REWARD_WEIGHT = 0.0
...
# Mandatory swing attempt. This removes the stand-still local optimum: after a
# short grace period, the episode is considered failed if neither the arm nor
# the whip has built meaningful speed. The terminal penalty is intentionally
# large enough that "do nothing" is worse than exploring a swing.
MANDATORY_SWING_ENABLED = True
SWING_CHECK_START_TIME_S = 0.45
SWING_FAIL_TIME_S = 1.25
MIN_ARM_SPEED_FOR_SWING_RAD_S = 0.55
MIN_WHIP_SPEED_FOR_SWING_M_S = 0.75
LOW_SWING_STEP_PENALTY = -3.0
NO_SWING_TERMINAL_PENALTY = -450.0
NO_SWING_TRUNCATE_ENABLED = True

# ============================================================
# TARGET-AIMED SWING REQUIREMENT / NO RANDOM-SWING FARMING
# ============================================================
# The previous mandatory-swing logic could be satisfied by whipping in any
# direction. These values make a swing count only if the whip tip is moving
# toward the target. Random fast motion gets no reward and can receive a
# penalty. This directly targets the loophole where the arm whips somewhere
# else just to collect movement/swing rewards.
MIN_DIRECTED_WHIP_SPEED_TOWARD_TARGET_M_S = 0.55
MIN_DIRECTED_SWING_ALIGNMENT = 0.35  # cosine-like: 1.0 = exactly toward target
RANDOM_SWING_SPEED_THRESHOLD_M_S = 0.80
RANDOM_SWING_AWAY_PENALTY_WEIGHT = 4.0
RANDOM_SWING_MISALIGNMENT_PENALTY_WEIGHT = 2.0

# The only large hit reward now requires actual target contact. Close misses can
# still provide small shaping rewards, but they cannot be mistaken for a true hit.
CONTACT_REQUIRED_FOR_HIT_REWARD = True
...
# Trainer can temporarily force the center target during early training so the
# policy first learns a real swing primitive before learning to aim across the
# full target box. The trainer toggles this through set_fixed_center_training_mode().
FIXED_CENTER_TRAINING_TARGET = TARGET_CENTER.copy()
...
class WhipWorldEnv(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        self._swing_attempted = False
        self._no_swing_failure = False
        self._mandatory_swing_penalty = 0.0
        self._directed_swing_velocity = 0.0
        self._directed_swing_alignment = 0.0
        self._random_swing_penalty = 0.0
        self._force_fixed_center_training = False
    ...
    def set_fixed_center_training_mode(self, enabled):
        """
        Called by SAC_trainer.py.

        enabled=True during the initial swing-pretraining phase. The target is
        fixed at FIXED_CENTER_TRAINING_TARGET so the policy learns a reliable
        whip/swing motion before it must solve the full random target box.
        """
        self._force_fixed_center_training = bool(enabled)
    ...
    def _resolve_target_position_from_options(self, options):
        ...
        if getattr(self, "_force_fixed_center_training", False):
            self._last_target_was_randomized = False
            return FIXED_CENTER_TRAINING_TARGET.copy()
    ...
    def _get_info(self):
        ...
        return {
            ...
            "swing_attempted": bool(self._swing_attempted),
            "no_swing_failure": bool(self._no_swing_failure),
            "mandatory_swing_penalty": float(self._mandatory_swing_penalty),
            "directed_swing_velocity": float(getattr(self, "_directed_swing_velocity", 0.0)),
            "directed_swing_alignment": float(getattr(self, "_directed_swing_alignment", 0.0)),
            "random_swing_penalty": float(getattr(self, "_random_swing_penalty", 0.0)),
            "fixed_center_training_mode": bool(getattr(self, "_force_fixed_center_training", False)),
        }
    ...
    def calc_reward(self):
        ...
        directed_alignment = float(velocity_toward_target / (whip_speed + 1e-6))
        self._directed_swing_velocity = velocity_toward_target
        self._directed_swing_alignment = directed_alignment
        ...
        if (
            ...
            # Reward only target-aimed speed. A fast random swing has low/negative
            # alignment and receives no directed reward.
            directed_swing_reward = (
                DIRECTED_SWING_REWARD_WEIGHT
                * max(0.0, velocity_toward_target)
                * max(0.0, directed_alignment)
            )
        ...
        if (
            ...
            # Multiply by alignment so random high-speed motion is not rewarded.
            aimed_swing_hint_reward = (
                AIMED_SWING_HINT_WEIGHT
                * max(0.0, velocity_toward_target)
                * max(0.0, directed_alignment)
            )
        ...
        if (
            self._distance_to_target < 0.45
            and velocity_toward_target > 0.0
            and directed_alignment > 0.20
        ):
        ...
        if velocity_toward_target > 0.0 and directed_alignment > 0.20:
            multi_joint_reward = MULTI_JOINT_REWARD_WEIGHT * (active_joint_count / 6.0)
        else:
            multi_joint_reward = 0.0
        ...
        random_swing_penalty = 0.0
        if (
            elapsed_time >= SWING_CHECK_START_TIME_S
            and not self._whip_target_contact
            and whip_speed > RANDOM_SWING_SPEED_THRESHOLD_M_S
        ):
            if velocity_toward_target <= 0.0:
                random_swing_penalty = -RANDOM_SWING_AWAY_PENALTY_WEIGHT * min(whip_speed, 4.0)
            elif directed_alignment < MIN_DIRECTED_SWING_ALIGNMENT:
                random_swing_penalty = (
                    -RANDOM_SWING_MISALIGNMENT_PENALTY_WEIGHT
                    * (MIN_DIRECTED_SWING_ALIGNMENT - directed_alignment)
                    * min(whip_speed, 4.0)
                )
        self._random_swing_penalty = random_swing_penalty
        ...
        # -------------------------------------------------
        if CONTACT_REQUIRED_FOR_HIT_REWARD:
            effective_hit = bool(self._whip_target_contact)
        else:
        ...
        mandatory_swing_penalty = 0.0
        if (
            MANDATORY_SWING_ENABLED
            and elapsed_time >= SWING_CHECK_START_TIME_S
            and not self._swing_attempted
            and not effective_hit
        ):
            # Per-step pressure after the grace period. This makes "stand still"
            # actively bad, but still allows the first part of the episode to
            # prepare/load the whip.
            mandatory_swing_penalty = LOW_SWING_STEP_PENALTY

        no_swing_terminal_penalty = 0.0
        if getattr(self, "_no_swing_failure", False):
            no_swing_terminal_penalty = NO_SWING_TERMINAL_PENALTY

        self._mandatory_swing_penalty = mandatory_swing_penalty + no_swing_terminal_penalty
    ...
    def step(self, action):
        ...
        self._update_derived_observation_values()
        elapsed_time = self._current_loop * self.MUJOCO_STEP_SIZE
        arm_speed_for_swing = float(np.linalg.norm(self.data.qvel[:6]))
        whip_speed_for_swing = float(np.linalg.norm(self._whip_velocity))
        swing_target_vec = self._target_position - self._whip_position
        swing_target_dist = float(np.linalg.norm(swing_target_vec))
        swing_target_dir = swing_target_vec / (swing_target_dist + 1e-9)
        directed_swing_velocity = float(np.dot(self._whip_velocity, swing_target_dir))
        directed_swing_alignment = float(directed_swing_velocity / (whip_speed_for_swing + 1e-6))
        self._directed_swing_velocity = directed_swing_velocity
        self._directed_swing_alignment = directed_swing_alignment
        if (
            MANDATORY_SWING_ENABLED
            and elapsed_time >= SWING_CHECK_START_TIME_S
            and directed_swing_velocity >= MIN_DIRECTED_WHIP_SPEED_TOWARD_TARGET_M_S
            and directed_swing_alignment >= MIN_DIRECTED_SWING_ALIGNMENT
        ):
            self._swing_attempted = True
        ...
        if CONTACT_REQUIRED_FOR_HIT_REWARD:
            self._safe_target_contact = bool(self._whip_target_contact and not self._unsafe_robot_failure)
        else:
        ...
        if (
            MANDATORY_SWING_ENABLED
            and NO_SWING_TRUNCATE_ENABLED
            and elapsed_time >= SWING_FAIL_TIME_S
            and not self._swing_attempted
            and not self._safe_target_contact
            and not self._whip_target_contact
        ):
            self._no_swing_failure = True
            truncated = True
