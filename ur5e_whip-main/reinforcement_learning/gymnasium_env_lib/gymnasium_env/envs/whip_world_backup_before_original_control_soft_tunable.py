...
class WhipWorldEnv(gym.Env):
    ...
    def calc_reward(self):
        ...
        # -------------------------------------------------
        # 6. Original smoothness penalties + very light hardware/mobility shaping
        # -------------------------------------------------
        # These first three terms are intentionally kept close to the older
        # working environment so the policy is not discouraged from making a
        # fast whipping motion.
        ...
        # UR5e documented maximum joint speed is 180 deg/s = pi rad/s for each
        # axis. This penalty is deliberately tiny and only activates above that
        # limit. It should guide the policy away from unrealistic velocities
        # without making it afraid to move.
        velocity_ratio = np.abs(self.data.qvel[:6]) / (self.UR5E_MAX_JOINT_SPEED_RAD_S + 1e-9)
        velocity_excess = np.maximum(0.0, velocity_ratio - 1.0)
        ur5e_speed_limit_penalty = -0.02 * np.sum(np.square(velocity_excess))

        # Very slight reward for involving more joints. This is intentionally
        # small so it does not replace the real objective: hit the target.
        joint_participation_reward = 0.0
        if self._distance_to_target > 0.20 and not self._whip_target_contact:
            joint_participation_reward = 0.005 * min(self._active_joint_count, 5)
