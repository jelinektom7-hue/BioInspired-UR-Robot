...
POLICY_ACTION_GAIN = 2.25
MAX_APPLIED_ACTION_RAD = 0.075
SETTLE_STEPS = 750
...
# Distance/progress learning signal. Distance reward remains active, but the
# closest/near-hit bonuses are only paid when the policy improves/crosses a
# threshold, so it cannot farm points by slowly hovering near the target.
DISTANCE_REWARD_WEIGHT = 4.0
DISTANCE_REWARD_DECAY = 5.0
PROGRESS_REWARD_WEIGHT = 180.0
PROGRESS_CLIP_NEG = -0.03
PROGRESS_CLIP_POS = 0.05
CLOSEST_IMPROVEMENT_REWARD_WEIGHT = 250.0

NEAR_HIT_BONUS_25CM = 10.0
NEAR_HIT_BONUS_18CM = 25.0
NEAR_HIT_BONUS_12CM = 60.0
NEAR_HIT_BONUS_08CM = 120.0
...
TIMEOUT_WITHOUT_HIT_PENALTY = -120.0
TIMEOUT_DISTANCE_PENALTY_WEIGHT = -120.0
LATE_NO_HIT_STEP_PENALTY = -0.25
LATE_NO_HIT_START_TIME_S = 1.25
...
class WhipWorldEnv(gym.Env):
    ...
    def calc_reward(self):
        ...
        # -------------------------------------------------
        # Distance/progress learning signal
        # -------------------------------------------------
        # Distance reward stays active, but it is kept moderate. The larger
        # reward is paid for actual progress/improvement, not for hovering.
        ...
        # -------------------------------------------------
        # Small motion/aggression shaping
        # -------------------------------------------------
        ...
        if (
            ...
            and self._shortest_distance_to_target > 0.10
        ...
        elapsed_time = self._current_loop * self.MUJOCO_STEP_SIZE
        if elapsed_time >= LATE_NO_HIT_START_TIME_S and not self._whip_target_contact:
