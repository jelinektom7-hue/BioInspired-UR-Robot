...
class WhipWorldEnv(gym.Env):
    ...
    def calc_reward(self):
        ...
        if self._arm_tip_ground_hit:
            safety_penalty += ARM_TIP_GROUND_PENALTY
        if self._arm_self_contact:
            safety_penalty += ARM_SELF_CONTACT_PENALTY
        if getattr(self, "_robot_ground_contact", False):
            safety_penalty += ROBOT_GROUND_CONTACT_PENALTY
        if unsafe_robot_failure:
            safety_penalty += UNSAFE_ROBOT_FAILURE_PENALTY
        ...
        if safe_effective_hit:
            ...
            safety_penalty += UNSAFE_HIT_EXTRA_PENALTY
