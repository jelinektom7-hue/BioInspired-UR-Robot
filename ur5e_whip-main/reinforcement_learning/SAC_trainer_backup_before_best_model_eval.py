...
# It strongly prioritizes actual/near hits over shaped reward.
BEST_SCORE_SUCCESS_WEIGHT = 10_000.0
BEST_SCORE_SIDE_HIT_WEIGHT = 2_500.0
BEST_SCORE_CLOSE_HIT_WEIGHT = 1_000.0
BEST_SCORE_REWARD_WEIGHT = 1.0
BEST_SCORE_MIN_DISTANCE_WEIGHT = 100.0
BEST_SCORE_FAILURE_PENALTY = 500.0
...
class BestWhipModelCallback(BaseCallback):
    ...
    def evaluate_current_policy(self, step: int):
        ...
        for target_index, target in enumerate(self.eval_targets):
            ...
            close_hit = bool(min_distance <= CLOSE_HIT_DISTANCE_M)
            success = bool(had_contact or close_hit)
            ...
            # Store the best-looking single rollout from this evaluation cycle.
            # This is for human inspection/debugging only; model selection uses
            # the aggregate score over all fixed targets.
        ...
        # Model-selection score. Contact/close hits dominate. Reward is secondary.
        score = (
            BEST_SCORE_SUCCESS_WEIGHT * contact_success_rate
            ...
            - BEST_SCORE_FAILURE_PENALTY * failure_rate
        )
